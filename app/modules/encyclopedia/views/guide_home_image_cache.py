...
class ImageService:
    ...
    The service deliberately separates cache hits/stores from synchronous loading:
    async callers can decode off the UI thread and only store the final QPixmap on
    the Qt thread, while small visible-only legacy callers can use ``load_scaled``.
    Catalogues never own decoded images.
    """
    ...
    def store_scaled(self, path: Path, size: QSize, pixmap: QPixmap) -> QPixmap:
        ...
        with self._lock:
            ...
            # complete shared cache just to retain themselves.
    ...
    def load_scaled(self, path: Path, size: QSize) -> QPixmap:
        """Load one already-requested visible image, reusing the shared cache.

        This synchronous path is intentionally for small visible-only callers.
        Heavy Guide images use ``get_scaled``/``store_scaled`` around their async
        decode path instead of decoding here.
        """
