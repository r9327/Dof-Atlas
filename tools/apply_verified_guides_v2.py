...
from app.modules.encyclopedia.services.guide_path_profiles import ORDER_QUESTS
...
def validate_exact_live(builder: GuideCatalogBuilder, guide_id: str, payload: dict[str, Any]) -> None:
    ...
    fatal = [error for error in errors if "invalide" in normalize_text(error) or "absent" in normalize_text(error)]
    if fatal:
        raise RuntimeError(f"{guide_id}: validation provider: {fatal[:10]}")
...
def validate_alignment_live(builder: GuideCatalogBuilder, side: str, payload: dict[str, Any]) -> None:
    ...
    expected_titles = sorted(ORDER_QUESTS[side], key=normalize_text)
    part_titles = [part.title for part in guide.parts]
    normalized_titles = [normalize_text(title) for title in part_titles]
    indexes = []
    for title in expected_titles:
        key = normalize_text(title)
        if key not in normalized_titles:
            raise RuntimeError(f"{guide_id}: étape d'Ordre absente: {title}")
        indexes.append(normalized_titles.index(key))

    if indexes != sorted(indexes) or min(indexes) < 5:
        raise RuntimeError(
            f"{guide_id}: les Ordres doivent être 3 étapes séparées après les 5 blocs d'alignement; indexes={indexes}"
        )

    for title in expected_titles:
        part = next(part for part in guide.parts if normalize_text(part.title) == normalize_text(title))
        quest_ids = {
            int(step.entity_id)
            for step in part.steps
            if step.step_type == "quest" and step.entity_id is not None
        ...
        if len(quest_ids) != 5:
            raise RuntimeError(f"{guide_id}: {title}: {len(quest_ids)} quêtes visibles au lieu de 5")

    if len({int(value) for value in guide.quest_ids}) != 115:
        raise RuntimeError(f"{guide_id}: total visible différent de 115 (100 alignement + 15 Ordres)")
...
def print_audit(result: Any) -> None:
    ...
    print()
    print("=" * 104)
    print("DOFUS ATLAS — GUIDES V2 — AVANT / APRÈS")
    print("=" * 104)
...
def main() -> None:
    ...
    if not args.apply:
        ...
        print(r"Application: py -3.13 .\tools\apply_verified_guides_v2.py --apply")
    else:
        backup_dir = ARTIFACTS_DIR / f"guides_v2_backup_{stamp}"
    ...
    report_path = ARTIFACTS_DIR / f"guides_v2_report_{stamp}.json"
...
if __name__ == "__main__":
    try:
        ...
        print("\nERREUR V2 — aucun exit PowerShell n'est exécuté.")
