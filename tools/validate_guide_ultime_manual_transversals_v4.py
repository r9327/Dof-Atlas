...
def main() -> None:
    parser = argparse.ArgumentParser(description="Valide Bonta1-60, Ordres20/40/60, Ocre et les contrats partagés 70-120.")
    ...
    bonta_row = next((row for row in transversal_rows if row.get("id") == "bonta_1_60"), None)
    bonta: dict[str, Any] = {}
    if bonta_row is None:
        hard.append({"code": "bonta_1_60_missing_from_manifest"})
    else:
    ...
    if bonta:
        if bonta.get("quest_range") != [1, 60]:
            hard.append({"code": "bonta_range_invalid", "expected": [1, 60], "actual": bonta.get("quest_range")})
        ...
        for required in ("BNT-07", "BNT-15", "BNT-19", "BNT-21", "BNT-23", "BNT-25"):
        ...
        tanukoui_text = route_text(by_id.get("BNT-23"))
        if "tanukoui san" not in tanukoui_text or "ocre" not in tanukoui_text:
            hard.append({"code": "tanukoui_shared_contract_incomplete"})
        b57_text = route_text(by_id.get("BNT-24"))
        for token in ("bucheron", "alchimiste", "sadida", "fleurs"):
            if token not in b57_text:
                hard.append({"code": "bonta57_social_gate_missing", "token": token})
    ...
    order60, names60 = validate_order_route(hard, conditional_rows, "bonta_order_rank60", 80, must_keep_order=True)
    if order20 and order40 and order60:
        keys20 = set(order20.get("options", {}))
        if set(order40.get("options", {})) != keys20 or set(order60.get("options", {})) != keys20:
            hard.append({"code": "order_keys_changed_across_paliers"})
        rank60_manifest = next((row for row in conditional_rows if row.get("id") == "bonta_order_rank60"), {})
        if rank60_manifest.get("same_order_as_rank20") is not True or rank60_manifest.get("same_order_as_rank40") is not True:
            hard.append({"code": "manifest_does_not_lock_same_order_at_60"})
        spirit = (order60.get("options") or {}).get("esprit salvateur") or {}
        if normalize_text("Chêne Mou") not in normalize_text(json.dumps(spirit, ensure_ascii=False)):
    ...
    if not ocre_name or not ocre_path.is_file():
        ...
        for required in ("Damadrya", "Tanukouï San", "Chêne Mou", "Minotoror", "Maître Corbac"):
            if required not in flattened:
                hard.append({"code": "ocre_future_boss_missing", "boss": required})
    ...
    if l100:
        ...
        require_quests(hard, by_id.get("L100-15"), "L100-15", {"Sous le bois de sa colère", "Le Saule du Promeneur", "Rester planté là", "Au nom de l'Art"})
        damadrya_text = route_text(by_id.get("L100-15"))
        for token in ("damadrya", "ocre", "au nom de l art", "tanukoui"):
            if token not in damadrya_text:
                hard.append({"code": "damadrya_shared_contract_missing", "token": token})
    ...
    if not args.skip_catalog:
        try:
            ...
            index = {}
        if index:
            names = explicit_quest_names(bonta) | names20 | names40 | names60 | explicit_quest_names(l70) | explicit_quest_names(l100)
    ...
    result = {
        ...
        "order60_options": sorted((order60.get("options") or {}).keys()) if order60 else [],
